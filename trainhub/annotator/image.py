...
def img_qt_to_arr(img_qt: QtGui.QImage) -> NDArray[np.uint8]:
    ...
    bytes_ = img_qt.constBits().asstring(w * h * d // 8)
...
def apply_exif_orientation(image: PIL.Image.Image) -> PIL.Image.Image:
    ...
    if orientation == 1:
        ...
        # top-to-left mirror
        return PIL.ImageOps.mirror(image.transpose(PIL.Image.Transpose.ROTATE_270))
    elif orientation == 6:
        # rotate 270
        return image.transpose(PIL.Image.Transpose.ROTATE_270)
    elif orientation == 7:
        # top-to-right mirror
        return PIL.ImageOps.mirror(image.transpose(PIL.Image.Transpose.ROTATE_90))
    elif orientation == 8:
        # rotate 90
