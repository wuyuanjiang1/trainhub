...
- **大模型 API（在线）**：填一个 API Key 即可调用视觉大模型检测框，服务商开箱即用：
  ...
  - 模型会按项目标签列表输出候选矩形框，标注词（label）统一输出英文，原类别名存入标注的 `other_data.label_cn` 便于中英对照；
  - 检测提示词只填目标名（如「木棍」或「木棍、纸箱」）时，这些词就是唯一允许的输出标签——提示词提到几种目标就最多输出几种标签，程序会在解析时丢弃多余的类别；
  - 批量预标注时对话框会实时显示文字统计：已处理/检出率/空结果、token 用量（↑输入 ↓输出）、缓存命中率与估算费用（≈¥，按服务商空闲档单价计算，实际以账单为准；未内置价格的服务商只显示 token 计数）；
